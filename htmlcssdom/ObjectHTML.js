...
finaReport: function() {
    ...
    for (var i = 0; i < dsr.departmentId.length; i++) {
       ...
       data += `Department ${dsr.departmentId[i]} - ${dsr.departments[i]}:`  + '\n';
        for (var j = 0; j < dsr.salaries[i].length; j++) {
            salaryTotal += parseInt (dsr.salaries[i][j]); 
            data +=  `Employee ID: ${dsr.employeeId[i][j]}` + "\n";
            data += `Name: ${dsr.employeeName[i][j]}` + "\n";
        }
        totalCompanySalary += salaryTotal;
        data += `Total combined salaries: ${salaryTotal}` + '\n'; 
    }
    data += `The companys entire salary total: ${totalCompanySalary}`;
...
runFunction : function() {
    this.loadDeptNames();
    setTimeout(this.loadDeptEmp, 100);
    setTimeout(this.loadDeptSalaries, 200);
    setTimeout(this.loadEmpName, 300);
    setTimeout(this.finaReport, 500);
